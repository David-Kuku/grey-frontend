...
export function useConvertView() {
  ...
  const { mutate: getQuote, isPending: quoting, error: quoteError } = useGetQuote()
  const { mutate: execute, isPending: executing, error: execError } = useExecuteConversion()
  ...
  function handleGetQuote(e: React.FormEvent) {
    ...
    getQuote(
      ...
      { onSuccess: (q) => { setQuote(q); setStep('confirm') } },
    )
  }
  ...
  return {
    ...
    quoting, quoteError,
    executing, execError,
    handleGetQuote, handleExecute, handleExpire,
  }
}
